...
DiskManager::DiskManager(const std::string &file)
    : fileName(file), nextPageId(0) {
  dbIo.open(file, std::ios::binary | std::ios::in | std::ios::out);

  // If file doesn't exist, create it
  // We could also check the current directory using std::filesystem if the file
  // exists and then decide accordingly But that could lead to TOC-TOU race
  // condition if we are not careful with the implementation
  if (!dbIo.is_open()) {
    dbIo.clear();
