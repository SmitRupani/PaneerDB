...
void Engine::executeSelect(SelectStatement* stmt) {
  ...
  for (auto* col : selectColumns) headers.push_back(col->name);
  ...
  while (currentPageId != -1) {
    ...
    for (slot_id_t i = 0; i < slotCount; ++i) {
      ...
      if (tupleData.empty()) continue; // lazy deleted
      ...
      if (stmt->filter != nullptr) {
        try {
          ...
          matches = std::visit([](const auto &v) -> bool {
  ...
  for (size_t i = 0; i < headers.size(); ++i) {
    std::cout << " " << headers[i];
    for (size_t p = 0; p < colWidths[i] - headers[i].length(); ++p) std::cout << " ";
    std::cout << " |";
  }
  std::cout << "\n";
  printBorder();
  ...
  for (size_t r = 1; r < results.size(); ++r) {
    ...
    for (size_t i = 0; i < selectColumns.size(); ++i) {
      ...
      std::cout << " " << val;
      for (size_t p = 0; p < colWidths[i] - val.length(); ++p) std::cout << " ";
      std::cout << " |";
    }
    std::cout << "\n";
  }
